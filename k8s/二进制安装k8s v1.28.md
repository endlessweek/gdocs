...
| 主机名称 | IP地址    | 说明       | 软件                                                         |
| -------- | --------- | ---------- | ------------------------------------------------------------ |
|     deploy    | 192.168.1.60 | 外网节点   | 下载各种所需安装包 |
...
# 修改静态的IPv4地址
nmcli con mod eth0 ipv4.addr 192.168.0.31/24 ipv4.gateway  192.168.0.1 method manual ipv4.dns "8.8.8.8" connetion.autoconnect yes && nmcli con up eth0
...
```shell
hostnamectl set-hostname k8s-master01 --static
...
# 对于 CentOS 7，这边只保留一个BASE的repo然后修改即可
...
# 下载全量依赖包，新建目录/data/centos7/,在该目录下操作
...
```shell
# 原方法有点复杂，只需要在部署节点生成公私密钥文件，然后ssh-copy-id到一台内网机将公钥写入authorized_keys 文件，然后将该公钥文件和本机/root/.ssh/id_rsa考入到所有需要免密的机器即可，同时注意同步hosts文件
...
内网机可以直接去以下网址下载内核
http://mirrors.coreix.net/elrepo-archive-archive/kernel/el7/x86_64/RPMS/  #内核下载地址
...
```shell
# 对于CentOS7离线安装
# yum install /data/centos7/ipset-*.el7.x86_64.rpm /data/centos7/lm_sensors-libs-*.el7.x86_64.rpm  /data/centos7/ipset-libs-*.el7.x86_64.rpm /data/centos7/sysstat-*.el7_9.x86_64.rpm  /data/centos7/ipvsadm-*.el7.x86_64.rpm  -y
...
EOF
```
### 1.19.在部署节点搭建简单镜像registry仓库
```shell
#用docker方式搭建，docker部署方法参考本章2.2，下面只写部署registry的方法
docker pull registry  #直接拉取registry镜像
docker run -itd  --restart always --name jason-hub -p 5000:5000 -v /opt/data/registry:/var/lib/registry registry  #在5000端口启动并将目录映射
vi /etc/docker/daemon.json
添加 "insecure-registries": ["本机ip:5000"]
systemctl restart docker
#推送测试
docker pull m.daocloud.io/registry.k8s.io/pause:3.8 #拉取镜像到本地 
docker tag m.daocloud.io/registry.k8s.io/pause:3.8 本机ip:5000/pause:3.8 #修改tag
docker push 本机ip:5000/pause:3.8 #推送镜像到私有仓库
#查看本地仓库镜像
[root@deploy ~]# curl -XGET 127.0.0.1:5000/v2/_catalog
{"repositories":["pause"]}
#查看某一镜像tag
[root@deploy ~]# curl -XGET 127.0.0.1:5000/v2/pause/tags/list
{"name":"pause","tags":["3.8"]}
#其他节点拉取镜像，由于是非安全模式，docker运行时的需要修改docker.json，containerd 需要加参数--insecure-registry
docker pull 192.168.192.4:5000/pause:3.8
#containerd运行时需要添加/etc/containerd/config.toml中以下内容，tls和config_path不能同时设置，需要注释掉
[plugins."io.containerd.grpc.v1.cri".registry]
     # config_path = "/etc/containerd/certs.d"
[plugins."io.containerd.grpc.v1.cri".registry.configs]       
[plugins."io.containerd.grpc.v1.cri".registry.configs."registry.jason.io:5000"]         [plugins."io.containerd.grpc.v1.cri".registry.configs."registry.jason.io:5000".tls]
            insecure_skip_verify = true  [plugins."io.containerd.grpc.v1.cri".registry.mirrors]       [plugins."io.containerd.grpc.v1.cri".registry.mirrors."registry.jason.io:5000"]
          endpoint = ["http://registry.jason.io:5000"]
#拉取测试，nerdctl还是需要加参数，crictl可以直接拉 
nerdctl -n=k8s.io pull registry.jason.io:5000/pause:3.8 --insecure-registry
crictl pull registry.jason.io:5000/nginx
...
```
###2.1.7 安装nerdctl作为containerd管理工具
```shell
#nerdctl有与docker相似的语法
...
### 2.3.3将组件发送至其他k8s节点，master和node节点所需组件不一样
...
```shell
# 请查看Github仓库 或者进行获取已经打好的包，整合包中有所有证书制作所需的json文件
...
```shell
# 写入生成证书所需的配置文件，这个根证书配置文件后面的kubernetes也需要
...
# -hostname=xxxx：指定要为etcd生成证书的主机名和IP地址列表，单master的话自己删除其他信息
...
# 10.96.0.1是service网段的第一个地址，需要计算，这个必须要有。192.168.0.36为高可用vip地址，单master就写单机地址
# 若没有IPv6 可删除可保留 
...
# - `-config=ca-config.json`：指定证书生成的配置文件路径，配置文件中包含了证书的有效期、加密算法等信息，这个用之前生成的即可
# - `-hostname=10.96.0.1,192.168.0.36,127.0.0.1,fc00:43f4:1eea:1::10`：指定证书的主机名或IP地址列表，10.96.0.1是服务地址，单master也需要写
...
### 6.1.1master01节点配置，单master注意修改相应ip
```shell
...
cd bootstrap  #用整合包里的bootstrap目录，里面有yaml文件
...
# token的位置在bootstrap.secret.yaml，如果修改的话到这个文件修改，在整合包中，可以直接用，也可以根据下面的内容自己创建
mkdir -p /root/.kube ; cp /etc/kubernetes/admin.kubeconfig /root/.kube/config

#bootstrap.secret.yaml内容如下，**切记执行，别忘记！！！**
...
## 8.2.kubelet配置，master和node节点都要配置
...
# 没有网络可以先下载calico的全量镜像离线包
wget https://github.com/projectcalico/calico/releases/download/v3.26.1/release-v3.26.1.tgz
#然后查看yaml文件中用到了哪些镜像，将所需镜像导入即可
[root@master1 calico]# cat calico.yaml | grep image:
...
[root@master1 calico]ctr -n k8s.io images import calico-cni.tar
[root@master1 calico]ctr -n k8s.io images import calico-typha.tar
[root@master1 calico]ctr -n k8s.io images import calico-node.tar
[root@master1 calico]ctr -n k8s.io images import calico-kube-controllers.tar
#也可以通过有公网的部署节点直接pull所需镜像然后上传私有仓库
docker pull docker.io/calico/cni:master
docker pull m.daocloud.io/docker.io/calico/node:master
docker pull docker.io/calico/kube-controller:master
...
cd /data/kubernetes-v1.28.0/cby #整合包里有
